...
let npcs = [];
// let worldObjects = [];
let worldObjects = [...gameObjects, ...flowers, ...npcs]; 
...
for (let i = 0; i < numOfGameObjects; i++) {
  ...
  let slug = {
    elmId: 'slug' + (i + 1),
    playerId: 'slug',
    posX: randomPosition(50, 350),
    posY: randomPosition(50, 350),
    width: 24,
    height: 36,
    objectType: 'npc',
    isStatic: true,
...
io.on('connection', function (socket) {
  ...
  socket.emit('inactiveSprites', gameObjects);

  socket.emit('placeSlugs', npcs);
